// app.js — UI stream client (normalized SSE, abort-safe)
// - No assistant_id here. Server picks.
// - Aborts prior stream when resubmitting.
// - Handles {delta|done|error} only.
...
async function streamChat(text) {
  ...
  if (!resp.ok || !resp.body) { live.innerHTML = '<span class="meta">Stream failed to start.</span>'; return; }
    ...
    while ((idx = buf.indexOf('\n\n')) >= 0) {
      ...
      if (event === 'delta') {
        try {
          ...
          if (j?.text) { collected += j.text; live.innerHTML = esc(collected).replace(/\n/g, '<br>'); }
        } catch {}
      } else if (event === 'error') {
